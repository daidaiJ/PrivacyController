...
| `exact` | 全词匹配（ASCII `\b` 边界） | 手机号、邮箱、API Key、英文标识符 |
| `substring` | 子串包含替换 | 地址、长文本片段、任意包含关系 |

### 模式详解

**token** - 中文分词匹配
- 使用 gse 中文分词引擎，按词切分后精确匹配
- `"张三"` 只匹配独立的「张三」，不匹配「张三丰」「张三李四」
- 适合中文姓名、中文短语

**exact** - 全词边界匹配
- 使用正则 `\b` 作为词边界，要求匹配内容前后是非单词字符（或行首/行尾）
- `"13800138000"` 匹配 `电话：13800138000` 但不匹配 `138001380001`
- `"sk-abc123"` 匹配 `key=sk-abc123` 但不匹配 `key=sk-abc1234`
- 适合手机号、邮箱、API Key、变量名等 ASCII 标识符

**substring** - 子串包含替换
- 只要文本中包含该子串就会被替换，无边界检查
- `"北京市朝阳区"` 会匹配 `我的地址是北京市朝阳区xxx路`
- 适合地址、长文本片段等需要宽松匹配的场景
